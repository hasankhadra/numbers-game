...
export async function POST(request: Request) {
  ...
  const systemPrompt = `You are playing a number guessing game. Generate a 4-digit number guess where all digits are different. Previous guesses and their feedback are provided as: number, exact matches (correct digit in correct position), and partial matches (correct digit in wrong position).`;
